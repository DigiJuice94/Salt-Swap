...
function readableError(value){if(value==null)return 'Scan failed.';if(typeof value==='string')return value;if(value instanceof Error)return value.message||'Scan failed.';if(typeof value==='object'){const preferred=value.message??value.error?.message??value.error??value.details??value.reason??value.statusText;if(preferred!=null&&preferred!==value)return readableError(preferred);try{return JSON.stringify(value)}catch{return 'Scan failed.'}}return String(value)}
...
async function scan(){
  ...
  try{
    ...
    if(!response.ok)throw new Error(data?.error||data?.message||`Salt scanner failed (HTTP ${response.status}).`);
    render(data);
  }catch(e){
