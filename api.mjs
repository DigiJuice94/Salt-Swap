...
async function socialNewsHandler(req,res){res.setHeader('Cache-Control','public, max-age=120, s-maxage=300, stale-while-revalidate=600');try{if(req.method!=='GET')return res.status(405).json({error:'Method not allowed'});const force=String(req.query.refresh||'')==='1';if(!force&&socialNewsCache.articles.length&&Date.now()-socialNewsCache.at<5*60*1000)return res.status(200).json({articles:socialNewsCache.articles,cached:true});const feeds=[['CoinDesk','https://www.coindesk.com/arc/outboundfeeds/rss/'],['Cointelegraph','https://cointelegraph.com/rss'],['Decrypt','https://decrypt.co/feed'],['The Block','https://www.theblock.co/rss.xml']];const batches=await Promise.all(feeds.map(([n,u])=>readCryptoRss(n,u))),seen=new Set(),articles=batches.flat().sort((a,b)=>new Date(b.publishedAt)-new Date(a.publishedAt)).filter(a=>{const k=a.title.toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();if(seen.has(k))return false;seen.add(k);return true}).slice(0,20);if(articles.length)socialNewsCache={at:Date.now(),articles};return res.status(200).json({articles:articles.length?articles:socialNewsCache.articles,cached:false})}catch(e){return res.status(500).json({error:errorText(e)})}}
