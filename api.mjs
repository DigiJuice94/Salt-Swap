...
function safeSolWallet(v){v=String(v||'').trim();try{return base58Bytes(v).length===32?v:''}catch{return''}}
function isEvmSocialWallet(v){return /^0x[a-fA-F0-9]{40}$/.test(String(v||'').trim())}
...
async function socialAuthHandler(req,res){res.setHeader('Cache-Control','no-store');try{
  if(req.method==='GET'){
    ...
    if(action==='challenge'){
      const wallet=safeSocialWallet(req.query?.wallet);if(!wallet)return res.status(400).json({error:'Valid supported wallet required.'});
...
async function socialProfileHandler(req,res){res.setHeader('Cache-Control','no-store');try{if(req.method==='GET'){if(req.query.username){const username=cleanUsername(req.query.username);if(!username)return res.status(400).json({error:'Valid username required.'});const owner=await kv('get',`salt:social:username:${username.toLowerCase()}`);return res.status(200).json({username,available:!owner,owner:owner||null})}const wallet=safeSocialWallet(req.query.wallet);if(!wallet)return res.status(400).json({error:'Valid supported wallet required.'});const raw=await kv('get',`salt:social:profile:${wallet}`);if(!raw)return res.status(200).json(null);const profile=JSON.parse(raw),viewer=await socialSessionWallet(req),following=Array.isArray(profile.following)?profile.following:[],followers=Array.isArray(profile.followers)?profile.followers:[];return res.status(200).json({...withTrenchesVerification(profile),followingCount:following.length,followersCount:followers.length,viewerFollowing:Boolean(viewer&&followers.includes(viewer))})}if(req.method!=='POST')return res.status(405).json({error:'Method not allowed'});const b=typeof req.body==='string'?JSON.parse(req.body):req.body||{},wallet=safeSocialWallet(b.wallet),message=String(b.message||''),signature=String(b.signature||''),evmChainId=String(b.evmChainId||'').toLowerCase();if(!wallet)return res.status(400).json({error:'Valid wallet is required.'});if(b.action==='follow'){const viewer=await socialSessionWallet(req);if(!viewer)return res.status(401).json({error:'Connect your Trenches Social wallet to follow profiles.'});if(viewer===wallet)return res.status(400).json({error:'You cannot follow your own profile.'});const viewerRaw=await kv('get',`salt:social:profile:${viewer}`),targetRaw=await kv('get',`salt:social:profile:${wallet}`);if(!viewerRaw)return res.status(403).json({error:'Create your Trenches profile first.'});if(!targetRaw)return res.status(404).json({error:'That Trenches profile no longer exists.'});const viewerProfile=JSON.parse(viewerRaw),targetProfile=JSON.parse(targetRaw),following=Array.isArray(viewerProfile.following)?viewerProfile.following:[],followers=Array.isArray(targetProfile.followers)?targetProfile.followers:[],isFollowing=following.includes(wallet);if(isFollowing){viewerProfile.following=following.filter(x=>x!==wallet);targetProfile.followers=followers.filter(x=>x!==viewer)}else{viewerProfile.following=[...new Set([...following,wallet])].slice(-5000);targetProfile.followers=[...new Set([...followers,viewer])].slice(-5000)}viewerProfile.updatedAt=new Date().toISOString();targetProfile.updatedAt=new Date().toISOString();await kv('set',`salt:social:profile:${viewer}`,JSON.stringify(viewerProfile));await kv('set',`salt:social:profile:${wallet}`,JSON.stringify(targetProfile));return res.status(200).json({following:!isFollowing,followingCount:viewerProfile.following.length,followersCount:targetProfile.followers.length,target:withTrenchesVerification(targetProfile)})}if(b.action==='edit'){
  ...
  const oldRaw=await kv('get',`salt:social:profile:${wallet}`);
  if(!oldRaw)return res.status(404).json({error:'Profile not found.'});
  const old=JSON.parse(oldRaw);

  const username=cleanUsername(b.username),
        avatar=String(b.avatar||''),
        avatarHash=String(b.avatarHash||'none'),
        bio=String(b.bio||'').trim().slice(0,160);

  if(!username)return res.status(400).json({error:'Use 3–24 letters, numbers, or underscores for your username.'});
  if(avatar&&(!avatar.startsWith('data:image/jpeg;base64,')||avatar.length>180000))return res.status(400).json({error:'Profile picture is invalid or too large.'});

  const digest=avatar?await crypto.subtle.digest('SHA-256',new TextEncoder().encode(avatar)):null,
        serverHash=digest?[...new Uint8Array(digest)].map(x=>x.toString(16).padStart(2,'0')).join(''):'none';
  if(serverHash!==avatarHash)return res.status(400).json({error:'Profile picture verification failed.'});

  const userKey=`salt:social:username:${username.toLowerCase()}`,
        existingOwner=await kv('get',userKey);

  if(existingOwner&&existingOwner!==wallet)return res.status(409).json({error:'That username is already taken. Pick another one.'});

  if(!existingOwner){
    const claimed=await kv('set',userKey,wallet,'nx');
    if(!claimed){
      const winner=await kv('get',userKey);
      if(winner!==wallet)return res.status(409).json({error:'That username was just claimed. Pick another one.'});
    }
  }

  if(old?.username&&old.username.toLowerCase()!==username.toLowerCase()){
    const oldKey=`salt:social:username:${old.username.toLowerCase()}`,
          oldOwner=await kv('get',oldKey);
    if(oldOwner===wallet)await kv('del',oldKey);
  }
...
async function swapBalanceHandler(req,res){
  ...
  try{
    if(req.method!=='GET')return res.status(405).json({error:'Method not allowed'});
    const wallet=safeSolWallet(req.query.wallet),mint=String(req.query.mint||'').trim(),key=process.env.HELIUS_API_KEY;
    if(!wallet)return res.status(400).json({error:'Valid Solana wallet required.'});
...
async function portfolioHandler(req,res){
  ...
  try{
    if(req.method!=='GET')return res.status(405).json({error:'Method not allowed'});
    const wallet=safeSolWallet(req.query.wallet),key=process.env.HELIUS_API_KEY;if(!wallet)return res.status(400).json({error:'Valid Solana wallet required.'});if(!key)return res.status(503).json({error:'Portfolio needs HELIUS_API_KEY configured in Vercel.'});
...
async function liveSocialAuthor(item,profileCache=new Map()){
  if(!item||typeof item!=='object')return item;
  const rawWallet=String(item.wallet||'').trim(),wallet=safeSocialWallet(rawWallet)||rawWallet;
  if(!wallet)return{...item};
  let profile=profileCache.get(wallet);
  if(profile===undefined){
    try{const raw=await kv('get',`salt:social:profile:${wallet}`);profile=raw?JSON.parse(raw):null}catch{profile=null}
    profileCache.set(wallet,profile);
  }
  if(!profile||typeof profile!=='object')return{...item};
  return{
    ...item,
    username:String(profile.username||item.username||'').trim(),
    avatar:typeof profile.avatar==='string'?profile.avatar:(item.avatar||'')
  }
}
async function liveSocialAuthors(items,profileCache=new Map()){
  return Promise.all((Array.isArray(items)?items:[]).map(item=>liveSocialAuthor(item,profileCache)))
}
async function publicSocialPostState(state,profileCache=new Map()){
  const [replies,quotes]=await Promise.all([
    liveSocialAuthors(state?.replies,profileCache),
    liveSocialAuthors(state?.quotes,profileCache)
  ]);
  return{...state,replies,quotes,likes:undefined}
}
async function enrichSocialReview(review,viewerWallet='',profileCache=new Map()){
  let r=await liveSocialAuthor(review,profileCache),needsMeta=!meaningfulSocialTokenName(r.tokenName)||!meaningfulSocialTokenSymbol(r.symbol)||!r.tokenImage||number(r.priceAtPost)==null,live={};
...
function openSeaPriceNumber(price){
  const raw=String(price?.value||'').trim(),decimals=Math.max(0,Math.min(36,Number(price?.decimals)||0));
  if(!/^\d+$/.test(raw))return null;
  const padded=raw.padStart(decimals+1,'0'),whole=decimals?padded.slice(0,-decimals):padded,fraction=decimals?padded.slice(-decimals).replace(/0+$/,''):'';
  const value=Number(fraction?`${whole}.${fraction}`:whole);
  return Number.isFinite(value)&&value>0?value:null;
}
async function fetchOpenSeaBestPrice(collection,identifier=''){
  const key=process.env.OPENSEA_API_KEY||'';
  if(!key)return null;
  const base='https://api.opensea.io/api/v2/listings/collection/'+encodeURIComponent(collection);
  const url=identifier?`${base}/nfts/${encodeURIComponent(identifier)}/best`:`${base}/best`;
  try{
    const response=await fetchJson(url,{headers:{accept:'application/json','x-api-key':key}},10000);
    const listing=identifier?response:(Array.isArray(response?.listings)?response.listings[0]:response);
    const current=listing?.price?.current,value=openSeaPriceNumber(current),currency=String(current?.currency||'').trim().slice(0,16);
    if(value===null)return null;
    return{price:value,currency,chain:String(listing?.chain||'').slice(0,40)};
  }catch{return null}
}
async function openSeaNftPriceHandler(req,res){
  res.setHeader('Cache-Control','public, max-age=60, s-maxage=120');
  try{
    if(req.method!=='GET')return res.status(405).json({error:'Method not allowed'});
    if(!process.env.OPENSEA_API_KEY)return res.status(503).json({error:'OpenSea pricing needs OPENSEA_API_KEY configured in Vercel.'});
    const collection=String(req.query?.collection||'').trim(),identifier=String(req.query?.identifier||'').trim();
    if(!/^[a-zA-Z0-9][a-zA-Z0-9._:-]{0,199}$/.test(collection))return res.status(400).json({error:'Valid OpenSea collection required.'});
    if(identifier&&!/^[a-zA-Z0-9][a-zA-Z0-9._:-]{0,199}$/.test(identifier))return res.status(400).json({error:'Valid NFT identifier required.'});
    let result=identifier?await fetchOpenSeaBestPrice(collection,identifier):null,label='Listed price';
    if(!result){result=await fetchOpenSeaBestPrice(collection);label='Floor price'}
    if(!result)return res.status(200).json({price:null,currency:'',chain:'',label:'Current price'});
    return res.status(200).json({...result,label});
  }catch(e){return res.status(Number(e?.status)||500).json({error:errorText(e)})}
}
...
async function socialCommunityFeedHandler(req,res){res.setHeader('Cache-Control','no-store');try{if(req.method!=='GET')return res.status(405).json({error:'Method not allowed'});const raw=await kv('get','salt:social:community-feed'),reviews=raw?JSON.parse(raw):[],viewer=await socialSessionWallet(req),sorted=reviews.sort((a,b)=>String(b.updatedAt).localeCompare(String(a.updatedAt))).slice(0,150),profileCache=new Map(),enriched=await Promise.all(sorted.map(r=>enrichSocialReview(r,viewer,profileCache)));return res.status(200).json({reviews:enriched})}catch(e){return res.status(Number(e?.status)||500).json({error:errorText(e)})}}
...
async function healthHandler(req,res){
  res.setHeader('Cache-Control','no-store');
  return res.status(200).json({ok:true,service:'The Trenches scanner',version:'1.12.14',providers:{helius:Boolean(process.env.HELIUS_API_KEY),opensea:Boolean(process.env.OPENSEA_API_KEY),birdeye:Boolean(process.env.BIRDEYE_API_KEY),jupiter:Boolean(process.env.JUPITER_API_KEY),zerox:Boolean(process.env.ZEROX_API_KEY),social:Boolean(process.env.UPSTASH_REDIS_REST_URL&&process.env.UPSTASH_REDIS_REST_TOKEN)}});
}
...
export default async function handler(req,res){
  try{
    ...
    if(route==='opensea-nft-price')return openSeaNftPriceHandler(req,res);
