...
async function fetchJson(url,options={},timeout=12000){const c=new AbortController();const t=setTimeout(()=>c.abort(),timeout);try{const r=await fetch(url,{...options,signal:c.signal});const text=await r.text();let data;try{data=text?JSON.parse(text):{}}catch{data={message:text}}if(!r.ok){const err=new Error(data?.message||data?.error||`HTTP ${r.status}`);err.status=r.status;throw err}return data}finally{clearTimeout(t)}}
...
module.exports=async function handler(req,res){
  ...
  }catch(e){console.error('Salt scan error',e);return res.status(e.status||500).json({error:e.message||'Salt scanner failed.'});}
};
