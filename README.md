# Salt Swap V1.10.40 — Crypto News Feed
...
- Preserves V1.10.40 Social profile layout and functionality.

# Salt Swap V1.10.40 — Social Reconnect Profile Fix

Fixes existing-profile wallet reconnect so Social always restores the full Profile layout instead of falling back to the legacy token-review screen. Also fixes the Social tab renderer scope bug and defaults reconnects to Profile. Preserves V1.10.40 banner persistence and all prior features.
...
## V1.10.40 — Salt Social account-first onboarding
...
V1.10.40 fixes: real full-width white Social canvas; hides floating background art on Social; removes global main width cap while Social is active; adds missing Vercel routes for social NFTs, holdings, and profile feed.
...
- Existing V1.10.40 layout and profile behavior are preserved.
...
- Preserves the V1.10.40 profile reconnect behavior and all prior Social profile features.

## V1.10.40 — Auto-Scrolling Checker Ticker
...
- Preserves V1.10.40 feed routing, CA-only custom tickers, auto-scroll, and all prior Social/profile behavior.


## V1.10.40 — Parachute meme coin background
