# Salt Swap V1.10.32 — Crypto News Feed
...
- Preserves V1.10.32 Social profile layout and functionality.

# Salt Swap V1.10.32 — Social Reconnect Profile Fix

Fixes existing-profile wallet reconnect so Social always restores the full Profile layout instead of falling back to the legacy token-review screen. Also fixes the Social tab renderer scope bug and defaults reconnects to Profile. Preserves V1.10.32 banner persistence and all prior features.
...
## V1.10.32 — Salt Social account-first onboarding
...
V1.10.32 fixes: real full-width white Social canvas; hides floating background art on Social; removes global main width cap while Social is active; adds missing Vercel routes for social NFTs, holdings, and profile feed.
...
- Existing V1.10.32 layout and profile behavior are preserved.
...
- Preserves the V1.10.32 profile reconnect behavior and all prior Social profile features.

## V1.10.32 — Auto-Scrolling Checker Ticker
...
- Preserves V1.10.32 feed routing, CA-only custom tickers, auto-scroll, and all prior Social/profile behavior.
